...
import { TrendingUp, TrendingDown, DollarSign, BarChart3, ArrowUp, ArrowDown, Target, AlertTriangle, Clock, Zap, Eye, ExternalLink, Download, RefreshCw, Play, Pause, Stop, Settings, Trash2, Edit, Plus, Filter } from "lucide-react"
...
import { cn } from "@/lib/utils"
import { useState } from "react"
import { Play, Pause, Stop, RotateCcw, Download, Settings, Trash2, Edit, Plus, Filter, ChevronUp, ChevronDown } from "lucide-react"
...
export default function BacktestPage() {
  ...
  const strategies = [
    { id: "momentum", name: "Momentum Strategy", description: "Buy stocks with strong 12-month momentum, rebalance monthly" },
    { id: "mean_reversion", name: "Mean Reversion", description: "Buy oversold stocks (RSI < 30), sell overbought (RSI > 70)" },
    { id: "breakout", name: "Breakout Strategy", description: "Buy on 20-day high breakout with volume confirmation" },
    { id: "factor_multi", name: "Multi-Factor", description: "Combined Value, Momentum, Quality factors with risk parity" },
    { id: "smart_money", name: "Smart Money Flow", description: "Follow institutional accumulation/distribution signals" },
  ]

  const mockResults = {
    momentum: {
      metrics: { cagr: 18.5, sharpe: 1.34, sortino: 1.89, maxDD: -14.2, calmar: 1.30, winRate: 58.3, profitFactor: 2.14, expectancy: 2.45, avgHold: 28, turnover: 1.45 },
      equity: [100, 102, 98, 105, 108, 112, 115, 110, 118, 122, 128, 135, 142, 145, 148, 152, 155, 158, 162, 158, 165, 170, 175, 178, 182],
      trades: [
        { date: "2023-01-15", ticker: "BBCA", type: "BUY", price: 8500, shares: 1000, pnl: 500000 },
        { date: "2023-02-20", ticker: "BMRI", type: "BUY", price: 5800, shares: 2000, pnl: 800000 },
        { date: "2023-03-15", ticker: "TLKM", type: "SELL", price: 3200, shares: 5000, pnl: -150000 },
        { date: "2023-04-10", ticker: "BBRI", type: "BUY", price: 5200, shares: 3000, pnl: 250000 },
        { date: "2023-05-15", ticker: "BBCA", type: "SELL", price: 9200, shares: 1000, pnl: 700000 },
      ]
    }
  }

  const strategy = mockResults[activeStrategy]
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Test and validate quantitative strategies on historical data</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">{strategies.find(s => s.id === activeStrategy)?.description}</p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-2 mt-2">
              <label className="flex items-center gap-1 text-sm">
                <input type="checkbox" className="rounded" defaultChecked /> Fees (0.15%+0.25%)
              </label>
              <label className="flex items-center gap-1 text-sm">
                <input type="checkbox" className="rounded" defaultChecked /> Slippage (0.1%)
              </label>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="flex justify-end gap-2 mb-6">
        <Button variant="outline" size="lg" onClick={runBacktest} disabled={isRunning} className="w-48">
          {isRunning ? (
            <>
        ...
        <Button variant="outline" size="lg" onClick={() => setResults(null)} disabled={isRunning} className="w-48">
              ...
              <CardContent>
                <div className="h-80 bg-muted/30 rounded-lg flex items-center justify-center text-muted-foreground">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground">Starting: 100 → Ending: 182 (+82%)</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="h-80 bg-muted/30 rounded-lg flex items-center justify-center text-muted-foreground">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground">Avg DD Duration: 45 days</p>
                    <p className="text-xs text-muted-foreground">Recovery Time: 67 days avg</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {[
                        { date: "2023-01-15", ticker: "BBCA", type: "BUY", price: 8500, shares: 1000, pnl: 500000, ret: 5.9, days: 45 },
                        { date: "2023-02-20", ticker: "BMRI", type: "BUY", price: 5800, shares: 2000, pnl: 800000, ret: 13.8, days: 62 },
                        { date: "2023-03-15", ticker: "TLKM", type: "SELL", price: 3200, shares: 5000, pnl: -150000, ret: -4.7, days: 28 },
                        { date: "2023-04-10", ticker: "BBRI", type: "BUY", price: 5200, shares: 3000, pnl: 250000, ret: 8.2, days: 18 },
                        { date: "2023-05-15", ticker: "BBCA", type: "SELL", price: 9200, shares: 1000, pnl: 700000, ret: 8.2, days: 62 },
                      ].map((trade) => (
                        <TableRow key={trade.date}>
                          ...
                          <TableCell className={`text-right font-medium ${trade.ret >= 0 ? "text-green-600" : "text-red-600"}`}>
